...
/**
 * Shared, dependency-free chrome for every pre-boot error screen.
 *
 * Included before Symfony can boot, so it must not rely on the autoloader,
 * Twig or the Webpack build. Callers populate the variables below, then
 * `require` this file.
 *
 * Expected in scope:
 *   string  $pageTitle
 *   string  $eyebrow
 *   string  $headingHtml   trusted markup for the <h1>
 *   string  $lede
 *   string  $panelLabel
 *   array   $panelRows     list of ['label', 'ok' => bool, 'value', 'target' => ?string]
 *   string  $hintHtml      trusted markup for the hint block
 *   string  $detail        raw technical text, shown only when $showDetail
 *   bool    $showDetail
 *   int     $statusCode
 */
...
<body>
<main>
    <span class="eyebrow"><span class="pulse"></span><?= $e($eyebrow) ?></span>
    ...
    <section class="panel" aria-label="Diagnostic">
        ...
        <div class="check">
            <span class="check-label"><?= $e($row['label']) ?></span>
            <span class="badge <?= $row['ok'] ? 'ok' : 'fail' ?>"><?= $row['ok'] ? 'OK' : 'Échec' ?></span>
            <div class="versions">
                <span class="v-current <?= $row['ok'] ? 'is-ok' : '' ?>"><?= $e($row['value']) ?></span>
                <?php if (!empty($row['target'])): ?>
                    <span class="arrow">&rarr;</span>
                    <span class="v-required"><?= $e($row['target']) ?></span>
                <?php endif; ?>
            </div>
        </div>
        <?php endforeach; ?>
