...
final class MediaModel extends BaseModel
{
    ...
    public static function fromEntity(mixed $entity, CoreLocatorInterface $coreLocator, bool $query = true): self
    {
        ...
        $response = new self(
            ...
            asCta: $intl && $intl->introduction && $intl->link && $intl->linkStyle && str_contains($intl->linkStyle, 'btn'),
            linkStyle: $intl && $intl->linkStyle ? $intl->linkStyle : null,
            radius: self::getContent('radius', $mediaRelation, true),
            rotation: self::getContent('rotation', $mediaRelation, true),
